...
async (req, res) => {
    ...
    if(instargram) profileFields.social.instargram = instargram;
...
router.get('/', async (req, res) => {
    try{
        const profiles = await Profile.find().populate('user', ['name', 'avatar', 'profession']);
        res.json(profiles);
    }catch(err){
        console.error(err.message);
        res.status(500).send('Server Error')
